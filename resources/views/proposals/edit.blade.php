        ...
        <form method="POST" action="{{ route('proposals.update', $proposal) }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="abstract" id="abstract" rows="4"
                    class="w-full border rounded px-3 py-2">{{ old('abstract', $proposal->abstract) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="objectives" id="objectives" rows="4"
                    class="w-full border rounded px-3 py-2">{{ old('objectives', $proposal->objectives) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="methodology" id="methodology" rows="4"
                    class="w-full border rounded px-3 py-2">{{ old('methodology', $proposal->methodology) }}</textarea>
                @error('methodology')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="flex gap-3">
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Save Changes</button>
                <a href="{{ route('proposals.show', $proposal) }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded">Cancel</a>
            </div>
        </form>
    </div>
</x-app-layout>
